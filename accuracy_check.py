...
try:
    from services import (
        get_bus_data, collect_bus_history, load_route_coords, filter_bus, map_index_df, calc_eta, 
        ORDERS_PER_KM, API_KEY, API_URL
    )
...
def get_expected_columns():
    cols = ["licence", "route", "stop_id", "stop_name", "stop_index", "registration_time", "arrival_time"]
    for t in PRED_INTERVALS: cols.extend([f"eta_min_T{t}", f"eta_ts_T{t}", f"error_T{t}"])
    return cols

def initialize_csvs():
    cols = get_expected_columns()
    with csv_lock:
        if not os.path.exists(ARCHIVE_CSV): pd.DataFrame(columns=cols).to_csv(ARCHIVE_CSV, index=False)
        if os.path.exists(ACTIVE_CSV): os.remove(ACTIVE_CSV)
...
def transfer_to_archive(row_index, df_current):
    try:
        ...
        return df_current.drop(row_index)
    except: return df_current
...
def register_new_bus_stops(route_name, licence, upcoming_stops, reg_time, current_speed, current_idx):
    ...
    if route_config and route_config.layover: current_layover_idx = route_config.layover.get('stop_index')

    for stop in upcoming_stops:
        t0_eta = pd.NA; t0_ts = pd.NA
        try:
            eta_val = calc_eta(current_idx, stop['index'], route_name, current_speed, layover_idx=current_layover_idx)
        ...
        row = {
            ...
            "arrival_time": pd.NA, "eta_min_T0": t0_eta, "eta_ts_T0": t0_ts, "error_T0": pd.NA 
        }
        for t in PRED_INTERVALS:
            if t == 0: continue 
            row[f"eta_min_T{t}"] = pd.NA; row[f"eta_ts_T{t}"] = pd.NA; row[f"error_T{t}"] = pd.NA
        new_rows.append(row)

    if new_rows:
        with csv_lock:
            try: pd.DataFrame(new_rows).to_csv(ACTIVE_CSV, mode='a', header=False, index=False)
            except: pass
...
def update_actual_arrival(route_name, licence, stop_index, actual_time):
    with csv_lock:
        try:
            ...
            mask = (df["licence"] == licence) & (df["route"] == route_name) & (df["clean_stop_idx"] == target_idx_str) & (df["arrival_time"].isna())
            if mask.any():
                row_idx = df[mask].index[-1]
                df.at[row_idx, "arrival_time"] = actual_time.isoformat()
                for t in PRED_INTERVALS:
                    ts_col = f"eta_ts_T{t}"; err_col = f"error_T{t}"
                ...
                if mask.any():
                    ...
                    for idx in active_indices:
                        for col, val in u['updates'].items():
                            curr = df.at[idx, col]
                            if pd.isna(curr) or str(curr).strip() == "": df.at[idx, col] = val
            df.drop(columns=['match_key', 'clean_stop_idx'], inplace=True)
            df.to_csv(ACTIVE_CSV, index=False)
        except: pass

def evaluate_bus_eta(route_name):
    print(f"[SYSTEM] Monitoring: {route_name}")
    tracked_buses = {} 
    try: route_config = direction_map[route_name]
    except: return
    current_layover_idx = None
    if route_config and route_config.layover: current_layover_idx = route_config.layover.get('stop_index')
        ...
        try:
            ...
            if not mapped_df.empty:
                for _, bus_row in mapped_df.iterrows():
                    ...
                    if not is_new:
                        if current_idx < tracked_buses[licence]["last_pos"] - (ORDERS_PER_KM * 2): is_new = True 
                    ...
                    if is_new:
                        ...
                        tracked_buses[licence] = {
                            ...
                            "last_pos": current_idx, "processed_intervals": {0}, "start_time": now,
                            "last_seen_time": now, "last_move_time": now, "is_stalled": False
                        }
                    else: tracked_buses[licence]["last_seen_time"] = now

                    bus_state = tracked_buses.get(licence)
                    if not bus_state: continue
                    if velocity > 0.5:
                        bus_state["last_move_time"] = now; bus_state["is_stalled"] = False
                    else:
                        if (now - bus_state["last_move_time"]).total_seconds() / 60 > STALL_THRESHOLD_MIN: bus_state["is_stalled"] = True
                        ...
                        for t in PRED_INTERVALS:
                            ...
                            if elapsed_min >= t and t not in bus_state["processed_intervals"]:
                                for stop_idx in bus_state["active_stop_indices"]:
                                    try:
                                        eta_min = calc_eta(current_idx, stop_idx, route_name, velocity, layover_idx=current_layover_idx)
                                        if eta_min is not None and eta_min >= 0:
                                            pred_ts = now + timedelta(minutes=eta_min)
                                            batch_updates.append({
                                                "licence": licence, "stop_index": stop_idx,
                                                "updates": { f"eta_min_T{t}": eta_min, f"eta_ts_T{t}": pred_ts.isoformat() }
                                            })
                                    except: pass
            ...
            for lic, state in tracked_buses.items():
                if lic not in current_active_licences:
                    if (now - state["last_seen_time"]).total_seconds() / 60.0 > OFFLINE_GRACE_PERIOD_MIN: keys_to_delete.append(lic)
            for lic in keys_to_delete: del tracked_buses[lic]
            if batch_updates: batch_update_predictions(route_name, batch_updates)
