...
'''CHECK HISTORY FOR MISSING BUSES'''
        ...
        for route_name in line_options:
            ...
            if route_config:
                for stop in route_config.stop_list.values():
                    ...
                    system_status.append({
                        "type": "stop", "route_name": route_name, 
                        "stop_id": str(stop['no']), "stop_name": stop['stop_name_eng'],
                        "id": f"{route_name}: {stop['no']}", "raw_id": f"{route_name}:{stop['no']}",
                        "status": s_data.get("status", "open"), "message": s_data.get("message", "-")
                    })
